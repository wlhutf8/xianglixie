var xlx = {
	...
	servicePage: function() {
		...
		var points = [ //10个坐标点
			[
				new BMap.Point(104.21599, 30.741361)
			],
		];
		...
		var myIcon = new BMap.Icon("images/dian.png", new BMap.Size(51, 81)); //自定义标注
...
$(function() {
	$("head").append('<link href="images/favicon.ico" rel="shortcut icon">');
	jsFun = $("#JS").attr('page');
	Starting_F();
	xlx.init();
...
function Starting_F() {
	//$("header").html("").load("header.html",header_load);
...
//	加载完成执行
	...
	function header_load(){
		...
		$(function(){
			//导航点击切换刷新页面
			if(_w < 768) {
				$("header .nav .nav-ul1 a.a1").attr("href", "javascript:void(0);");
	...
	//弹出框
	if(_w<=1024){
		$(".show-list .go_shop img").attr("src","images/go-shop1.png");
	}
}
